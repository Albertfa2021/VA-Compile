...
public class LHMAuraMenuSoundSource : MonoBehaviour {
    ...
    void Start()
    {
        auralizationMode = FindObjectOfType<VAUSoundSource>().gameObject.GetComponent<VAUAuralizationMode>();
        if (auralizationMode == null)
            Debug.Log("[VAU] No SoundSources in Scene.");
        auralizationMode.AuraStringChanged += AuralizationMode_AuraStringChanged;
        GetComponentInChildren<LHMSoundSourceSlider>().ActSoundSourceChanged += LHMAuraMenuSoundSource_ActSoundSourceChanged;
        RefreshAuraMode();
    }

    private void LHMAuraMenuSoundSource_ActSoundSourceChanged(VAUSoundSource actSoundSource)
    {
        auralizationMode = actSoundSource.gameObject.GetComponent<VAUAuralizationMode>();
        RefreshAuraMode();
    }
