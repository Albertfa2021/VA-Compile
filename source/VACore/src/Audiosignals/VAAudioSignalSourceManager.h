...
public:
	...
	IVAAudioSignalSource* RequestSignalSource( const std::string& sID, const ITASampleFrame** ppsbInputBuf = nullptr );
	...
	//! Stille zurückgeben
	const ITASampleFrame* GetSilenceBuffer( ) const;
...
private:
	...
	public:
		...
		ITASampleFrame sfOutputBuffer; // Puffer für nächsten Block Audiosamples
		...
		inline CAudioSignalSource( IVAAudioSignalSource* pSource, const std::string& sName, int iBlockLength, bool bManaged, bool bDynamic )
		    : sfOutputBuffer( pSource->GetNumChannels( ), iBlockLength, true )
		    , pSource( pSource )
		    , sName( sName )
		    , iRefCount( 0 )
		    , bManaged( bManaged )
