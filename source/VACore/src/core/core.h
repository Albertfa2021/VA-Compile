...
public:
	...
	bool GetSoundSourceMuted( const int iID ) const override;
	void SetSoundSourceMuted( const int iID, const bool bMuted = true ) override;
...
private:
	...
	std::atomic<long> m_lSyncModOwner; //!< ID des Thread der Synchronisierungs-Token hat
	...
	std::atomic<int> m_iGlobalAuralizationMode;
	...
	public:
		...
		inline CVAAudioRendererDesc( IVAAudioRenderer* pInstance ) : pInstance( pInstance ), pOutputRecorder( NULL ), pOutputDetector( NULL ) { };
	...
	std::atomic<int> m_iState; // Modaler Zustand (siehe core states)
	...
	std::atomic<float> m_fCoreClockOffset; // Offset der Core-Clock
