...
//! Implementierung eines Bewegungsmodells mit Sample-and-Hold Funktionalit�t
class CVASampleAndHoldMotionModel : public IVAMotionModel
{
public:
	inline CVASampleAndHoldMotionModel( ) : m_pLastState( nullptr ), m_dStartTime( 0 )
	{
		char buf1[255];
		sprintf( buf1, "SampleAndHoldMotionModel_Estimation_0x%08Xh.log", intptr_t( this ) );
		m_oEstimationDataLog.setOutputFile( buf1 );
		char buf2[255];
		sprintf( buf2, "SampleAndHoldMotionModel_Input_0x%08Xh.log", intptr_t( this ) );
	...
	{
		if( m_pLastState )
			m_pLastState->RemoveReference( );
	};

	inline void InputMotionKey( const CVAMotionState* pNewState )
	{
		if( m_dStartTime == 0 )
			m_dStartTime = pNewState->GetModificationTime( );
		if( m_pLastState )
			m_pLastState->RemoveReference( );
		...
		oLogItem.dTime = pNewState->GetModificationTime( ) - m_dStartTime; // Bessere Aufl�sung
...
private:
	...
	ITABufferedDataLogger<MotionLogDataOutput> m_oEstimationDataLog; //!< Logger Datum f�r VDL spezifische Prozess-Information
	...
	ITABufferedDataLogger<MotionLogDataInput> m_oInputDataLog; //!< Logger Datum f�r VDL spezifische Prozess-Information
};
