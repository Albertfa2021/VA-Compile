...
{
public:
	inline CVASharedMotionModel( ) : m_pImpl( nullptr ), m_bDeleteModel( false ) { };

	inline CVASharedMotionModel( IVAMotionModel* pMotionModel, bool bDeleteModel ) : m_pImpl( pMotionModel ), m_bDeleteModel( bDeleteModel ) { };
	...
	inline bool EstimatePosition( double dTime, VAVec3& vPos ) { return m_pImpl->EstimatePosition( dTime, vPos ); };

	inline bool EstimateOrientation( double dTime, VAVec3& vView, VAVec3& vUp ) { return m_pImpl->EstimateOrientation( dTime, vView, vUp ); };

	inline bool EstimateOrientation( double dTime, VAQuat& oOrient ) { return m_pImpl->EstimateOrientation( dTime, oOrient ); };

	inline IVAMotionModel* GetInstance( ) { return m_pImpl; };

	inline void Reset( ) { m_pImpl->Reset( ); };
