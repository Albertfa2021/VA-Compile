 ...
 */
template<typename T>
class CVAObjectContainer
{
public:
	// iEndID letztmögliche ID. Falls Überlauf -> Exception
	inline CVAObjectContainer( int iStartID = 1, int iLastID = static_cast<unsigned int>( -1 ) )
	    : m_iStartID( iStartID )
	    , m_iIDCount( iStartID )
	    , m_iIDLast( iLastID ) { };
	...
	inline int Release( int iID ) { return DecRefCount( iID ); };

	// Der Operator [] ist Alias für GetObject()
	inline T* operator[]( int iID ) const { return GetObject( iID ); };
