...
#define VA_PRINT( expr )                                                                \
	{                                                                                   \
		if( VA_STDOUT && ( VALog_GetLogLevel( ) >= IVAInterface::VA_LOG_LEVEL_QUIET ) ) \
		{                                                                               \
			VistaMutexLock oLock( VALog_getOutputStreamMutex( ) );                      \
			( *VA_STDOUT ) << "[ VAPrint   ] " << expr << std::endl;                    \
		}                                                                               \
	}
#define VA_ERROR( module, expr )                                                                                          \
	{                                                                                                                     \
		if( VA_STDOUT && ( VALog_GetLogLevel( ) >= IVAInterface::VA_LOG_LEVEL_QUIET ) )                                   \
		{                                                                                                                 \
			VistaMutexLock oLock( VALog_getOutputStreamMutex( ) );                                                        \
			( *VA_STDERR ) << "[ VAError   ][ " << std::right << std::setw( 20 ) << module << " ] " << expr << std::endl; \
		}                                                                                                                 \
	}
#define VA_WARN( module, expr )                                                                                           \
	{                                                                                                                     \
		if( VA_STDOUT && ( VALog_GetLogLevel( ) >= IVAInterface::VA_LOG_LEVEL_ERROR ) )                                   \
		{                                                                                                                 \
			VistaMutexLock oLock( VALog_getOutputStreamMutex( ) );                                                        \
			( *VA_STDOUT ) << "[ VAWarning ][ " << std::right << std::setw( 20 ) << module << " ] " << expr << std::endl; \
		}                                                                                                                 \
	}
#define VA_INFO( module, expr )                                                                                           \
	{                                                                                                                     \
		if( VA_STDOUT && ( VALog_GetLogLevel( ) >= IVAInterface::VA_LOG_LEVEL_INFO ) )                                    \
		{                                                                                                                 \
			VistaMutexLock oLock( VALog_getOutputStreamMutex( ) );                                                        \
			( *VA_STDOUT ) << "[ VAInfo    ][ " << std::right << std::setw( 20 ) << module << " ] " << expr << std::endl; \
		}                                                                                                                 \
	}
#define VA_VERBOSE( module, expr )                                                                                        \
	{                                                                                                                     \
		if( VA_STDOUT && ( VALog_GetLogLevel( ) >= IVAInterface::VA_LOG_LEVEL_VERBOSE ) )                                 \
		{                                                                                                                 \
			VistaMutexLock oLock( VALog_getOutputStreamMutex( ) );                                                        \
			( *VA_STDOUT ) << "[ VAVerbose ][ " << std::right << std::setw( 20 ) << module << " ] " << expr << std::endl; \
		}                                                                                                                 \
	}
#define VA_TRACE( module, expr )                                                                                          \
	{                                                                                                                     \
		if( VA_STDOUT && ( VALog_GetLogLevel( ) >= IVAInterface::VA_LOG_LEVEL_TRACE ) )                                   \
		{                                                                                                                 \
			VistaMutexLock oLock( VALog_getOutputStreamMutex( ) );                                                        \
			( *VA_STDOUT ) << "[ VATrace   ][ " << std::right << std::setw( 20 ) << module << " ] " << expr << std::endl; \
		}                                                                                                                 \
	}
...
public:
	...
	inline CVALogItem( ) : dTimestamp( 0 ) { };
...
{
public:
	inline CVARealtimeLogStream( ITAClock* pClock = ITAClock::getDefaultClock( ) ) : m_pClock( pClock )
	{
...
private:
	ITAClock* m_pClock;
	std::string m_sName;
	tbb::concurrent_queue<CVALogItem> m_qLog;
