 ...
 */
class CVAAudiostreamStateImpl
    : public CVAAudiostreamState
    , public ITAStreamInfo
{
public:
	inline CVAAudiostreamStateImpl( ) : ITAStreamInfo( ) { };
	inline CVAAudiostreamStateImpl( const ITAStreamInfo& base ) : ITAStreamInfo( base ) { i64Sample = base.nSamples; };
};
...
{
public:
	inline CVAAudiostreamTracker( ITADatasource* pSource, ITAClock* pClock, std::atomic<float>* pfClockOffset, std::atomic<long>* plSyncModOwner,
	...
	{
		VA_VERBOSE( "AudioStreaming", "Processing time: " << m_swProcessingTime.ToString( )
		                                                  << ", avg DSP load = " << m_swProcessingTime.mean( ) * m_dSamplerate / m_uiBlocklength * 1e2 << "%" );
	}

	// Not thread safe, not reentrant safe
	inline const CVAAudiostreamState& GetStreamState( ) const { return m_oState; }
...
private:
	...
	ITAClock* m_pClock;
	std::atomic<float>* m_pfClockOffset;
	std::atomic<long>* m_plSyncModOwner;
