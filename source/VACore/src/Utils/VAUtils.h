...
//! Ties a position with view/up orientation to a position and quaternion orientation for Vista types
...
//! Get azimuthal angle from a reference coordinate system defined by orientation and direction to target
double GetAzimuthFromDirection_DEG( const VAVec3& vView, const VAVec3& vUp, VAVec3 v3Direction );
...
//! Get elevation angle from a reference coordinate system defined by orientation and direction to target
double GetElevationFromDirection_DEG( const VAVec3& vUp, VAVec3 v3Direction );

//! Convert VAVec3 to VistaVector3D
VistaVector3D VAVec3ToVistaVector3D( const VAVec3& vaVec );

//! Convert VistaVector3D to VAVec3
...
{
public:
	inline void Add( const std::string& sLiteral, const T& value ) { m_mLiterals.insert( std::pair<std::string, T>( toUppercase( sLiteral ), value ) ); };
...
public:
	...
	inline std::string GetErrorPrefix( ) const { return m_sErrorPrefix; };
	...
	// Hook: Error handler
	// Customize to your needs
	inline virtual void Error( const std::string& sErrorMessage ) const { VA_EXCEPT1( m_sErrorPrefix + sErrorMessage ); };

	// --= Error messages =--

	inline void ErrorMissKey( const std::string& sKey ) const { Error( std::string( "Missing a key \"" ) + sKey + std::string( "\"" ) ); };
	...
	inline void ErrorEmptyKey( const std::string& sKey ) const { Error( std::string( "Key \"" ) + sKey + std::string( "\" must not be empty" ) ); };
	...
	inline void ReqBool( const std::string& sKey, bool& bValue, const CVALiterals<bool>* pLiterals = nullptr ) const
	{
		...
		if( pValue->IsString( ) )
		{
			...
			if( pLiterals && pLiterals->Translate( *pValue, bValue ) )
				return;
	...
	inline void ReqInteger( const std::string& sKey, int& iValue, const CVALiterals<int>* pLiterals = nullptr ) const
	{
		...
		{
			// Try to translate literals
			if( pLiterals && pLiterals->Translate( *pValue, iValue ) )
				return;
	...
	inline void ReqNumber( const std::string& sKey, double& dValue, const CVALiterals<double>* pLiterals = nullptr ) const
	{
		...
		{
			// Try to translate literals
			if( pLiterals && pLiterals->Translate( *pValue, dValue ) )
				return;
