...
{
public:
	CVAProfilerMeasure( const std::string& sMeasureName ) : m_sMeasureName( sMeasureName ), m_dStartTime( 0 ) { assert( !sMeasureName.empty( ) ); }
	...
	inline void cancel( ) { m_dStartTime = 0; }
	...
	{
		std::stringstream ss;
		ss << m_sMeasureName << ": min=" << timeToString( m_dAnalyzer.minimum( ) ) << ", avg=" << timeToString( m_dAnalyzer.mean( ) )
		   << ", max=" << timeToString( m_dAnalyzer.maximum( ) ) << ", std=" << timeToString( m_dAnalyzer.std_deviation( ) );
		return ss.str( );
	}
